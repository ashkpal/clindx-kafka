...
// PatientHandler is implemented by the application
type PatientHandler func(ctx context.Context, event *models.PatientEvent) error
...
func ConsumePatient(
	...
	log.Printf("[Kafka] Consuming patient from topic=%s", cfg.Topic)
	...
	for {
		select {
		case <-ctx.Done():
			log.Println("[Kafka] patient consumer shutting down")
			return nil

		default:
			msg, err := reader.FetchMessage(ctx)
			if err != nil {
				log.Printf("[Kafka] patient fetch error: %+v", err)
